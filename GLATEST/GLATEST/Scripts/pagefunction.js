...
window.defaultStatus = "Welcome to Uniplan";
...
var mouseY = 0;
	...
	function setnewcolor(source) {
	    ...
	    //透過onMouseOver動態改變GridView "整行"的顏色/底色(灰)
	    source.style.backgroundColor = "#cccccc";
	    _selcolor = source.style.backgroundColor;
	    if (_oldcolor == _selcolor) {
	        //如果指定的(顏色/底色)和原本相同時,則指定變成預設顏色(紫)
	        _oldcolor = "#CCCCFF";
	        source.style.backgroundColor = "#9999FF";
	    }
        _newcolor = source.style.backgroundColor;
    }

    //變成指定底色
    function setnewcolorTo(source, newcolor) {
        _oldcolor = source.style.backgroundColor;
        source.style.fontWeight = '';

        //透過onMouseOver動態改變GridView "整行"的顏色/底色
        source.style.backgroundColor = newcolor;
        _selcolor = source.style.backgroundColor;
        if (_oldcolor == _selcolor) {
            //如果指定的(顏色/底色)和原本相同時,則指定變成預設顏色(灰)
            _oldcolor = "#CCCCFF";
            source.style.backgroundColor = "#cccccc";
        }
        _newcolor = source.style.backgroundColor;
    ...
    function setoldcolor(source) {        
        if (source.style.backgroundColor == _newcolor)
            source.style.backgroundColor = _oldcolor;
    }

 function overShow(showDiv) {
     var showDiv = document.getElementById(showDiv);
     if (showDiv != null && showDiv != 'undefined') showDiv.style.display = showDiv.style.display == 'none' ? 'inline' : 'none';
 }
...
//  OpenDayCalendar()：打開日行事曆
///////////////////////////////////////////////////////////////////////////////////////////
function OpenDayWSCalendar(theSessionID, Connect, theDate, theCompany, theSeqNo, theDeptId, theEmployeeId, theCategory, theStatus) {
    var URL;
    var ReturnValue;
    URL = "Prompt/Calendar.aspx?theSessionID=" + theSessionID + "&Connect=" + Connect + "&theDate=" + theDate + "&theCompany=" + theCompany + "&theSeqNo=" + theSeqNo + "&theDeptId=" + theDeptId + "&theEmployeeId=" + theEmployeeId + "&theCategory=" + theCategory + "&theStatus=" + theStatus;
    window.open(URL, "日行事曆", "status=no, menubar=yes, scrollbars=yes, resizable=yes, copyhistory=yes, width=800, height=600");
}