...
(function ($) {
    $.fn.dropdownlist = function (opts) {
        ...
        return this.each(function () {
            ...
            if (settings.id == '') throw new Error('dropdownlist 的id不能為空。');
            if (settings.checkbox) {
                for (var _i = 0; _i < settings.requiredvalue.length; _i++) {
                    if ($.inArray(settings.requiredvalue[_i], settings.selected) == -1) {
                        throw new Error(settings.id + '錯誤：必须選擇的值（requiredvalue）没有設置為選中（selected）');
                    }
                }
                if (settings.selected.length > settings.maxchecked) throw new Error(settings.id + '錯誤：預設選中的值（selected）多于設置的最多同時選擇的值（maxchecked）');
            }
            ...
            if (settings.checkbox) {
                var tfoot = $('<tfoot><td colspan="' + settings.columns + '"><span class="info">最多可選' + settings.maxchecked + '項</span><span class="btn-area"><input type="button" value="確定" class="btn-ok"><input type="button" value="取消" class="btn-cancel"></span></td></tfoot>');
            ...
            objid.init = function (mdata, selectedValue) {
                ...
                selecttext = selecttext == '' ? '請選擇' : selecttext;
