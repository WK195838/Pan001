...
(function ($) {
    $.MoverBoxes = function (element, options) {
        ...
        var Moverhtml = function (Uid, dataleft, dataright,leftLabel, rightLabel) {
            ...
            $(tableTDSelect).attr("id", Uid+"selectorOne");
            $(tableTDSelect).attr("name", Uid+"selectorOne");
            ...
            for (var i = 0, len = options.dataleft.length; i < len; ++i) {
                ...
                $(optionEl).attr("id", Uid+"optId_" + dataObj[0]);
            ...
            $(inputTwoAll).attr('id', Uid + 'moveLeftAll');
            ...
            $(inputThreeAll).attr('id', Uid+'moveRightAll');
            ...
            $(tableTDSelectTwo).attr("id", Uid+"selectorTwo");
            $(tableTDSelectTwo).addClass("MoverBoxRight");
            $(tableTDSelectTwo).addClass("MoverBox");
            $(tableTDSelectTwo).attr("name", Uid+"selectorTwo");
            ...
            for (var i = 0, len = options.dataright.length; i < len; ++i) {
                ...
                $(optionEl2).attr("id", Uid+"optId_" + dataObj[0]);
        ...
        plugin.init = function () {
            ...
            var myMoverBox = new Moverhtml(options.Uid, options.dataleft, options.dataright, options.leftLabel, options.rightLabel);
            
            //add to dom
            $(element).html(myMoverBox.Moverhtml);

            $('#' + options.Uid + 'moveLeft').live('click', function () {
                moveLeftToright();
            });

            $('#' + options.Uid + 'moveRight').live('click', function () {
                moveRightToleft();
            });

            $('#' + options.Uid + 'moveRightAll').live('click', function () {
                movAllRightToleft();
            });
            $('#' + options.Uid + 'moveLeftAll').live('click', function () {
                moveAllLeftToRight();
            });
        }
        ...
        plugin.SelectedValues = function () {
            ...
            var values = [];
            $('#' + options.Uid + 'selectorTwo option').each(function (index) {
                values.push($(this).attr('value'));
            });
            return values;
        }

       plugin.SelectedText = function () {

            var values = [];
            $('#' + options.Uid + 'selectorTwo option').each(function (index) {
                values.push($(this).attr('text'));                
            });
            return values;
        }
        ...
        plugin.NotSelectedValues = function () {
            ...
            var values = [];
            $('#' + options.Uid + 'selectorOne option').each(function (index) {
                values.push($(this).attr('value'));
            });
            return values;
        ...
        var moveAllLeftToRight = function(){
            $('#' + options.Uid + 'selectorTwo option').each(function (index) {
                ...
                $('#' + options.Uid + 'selectorOne').append(that); 
            });
        }
        var movAllRightToleft = function () {
            $('#' + options.Uid + 'selectorOne option').each(function (index) {
                ...
                var that = $(this);
                $(that).removeClass('selected');
                $('#' + options.Uid + 'selectorTwo').append(that);
            });
        }
